...
import { BrioCollision } from "../math/BrioCollision.js";
import { create } from "../math/vec2.js";
export class BrioObject {
    // Basic properites
    #transform = new BrioTransform(create(0, 0), create(0, 0));
    ...
    set clonesInstantiatedValue(value) {
        if (!BrioObject.instanceOfObject) {
            throw new Error("The number of clones can't be hard coded, their amount increases automatically when new instances are created.");
        }
    ...
    static clone(gameObject) {
        const object = new BrioObject(gameObject.#name, gameObject.#sprite, gameObject.transform.layer);
        if (object.collision) {
