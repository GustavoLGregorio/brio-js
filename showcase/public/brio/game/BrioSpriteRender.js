...
export class BrioSpriteRender {
    ...
    #gameHeight;
    gameLastFPS;
    constructor(game, assets, canvasContext, gameScale, width, height, gameLastFPS) {
        this.#game = game;
        this.#assets = assets;
    ...
    renderObject(object) {
        if (!object || !object.transform.visibility || object.transform.opacity === 0)
            return;
        ...
        this.#ctx.translate(object.transform.position.x * this.#gameScale + offsetX, object.transform.position.y * this.#gameScale + offsetY);
        this.#ctx.scale(scaleX, scaleY);
        let translated = false;
        if (object.transform.rotation !== 0) {
            // this.#ctx.translate(object.transform.size.x / 2, object.transform.size.y / 2);
            this.#ctx.translate(object.transform.size.x * object.transform.pivot.x, object.transform.size.y * object.transform.pivot.y);
        ...
        // sprite rendering
        this.#ctx.drawImage(sprite.element, translated ? -object.transform.size.x * object.transform.pivot.x : 0, translated ? -object.transform.size.y * object.transform.pivot.y : 0, object.transform.size.x, object.transform.size.y);
    ...
    clearObject(gameObject) {
        ...
        this.#ctx.clearRect(gameObject.transform.position.x, gameObject.transform.position.y, gameObject.transform.size.x, gameObject.transform.size.y);
        this.#ctx.restore();
    }
    renderCollisions() {
    ...
    renderFPSOverlay(position, offset, size, backgroundColor, textColor) {
        if (!this.#ctx)
            return;
        let pos = { x: 0, y: 0 };
        const off = offset;
        ...
        switch (position) {
            ...
            case "right-top":
                ...
                break;
            case "right-center":
                pos = { x: rightX, y: centerY };
                break;
            case "right-bottom":
                pos = { x: rightX, y: bottomY };
                break;
        }
